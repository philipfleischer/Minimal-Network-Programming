...
def handle(client):
    while True:
        try:
            # Just making it point to the same place, due to looping problems
            msg = message = client.recv(1024)  # bytes 1024
            if not msg:
    ...
    while True:
        ...
        if nickname == "admin":
            ...
            if password != "adminpasswd":
                ...
                client.close()
                continue  # Not break, so we can continue on wron passwd
